...
export default function Clients() {
  ...
  useEffect(() => {
    async function load() {
      if (!user) return;
      // List only needs the fields rendered on the cards; the full row is
  ...
  async function showHistory(client: Client) {
    ...
    const { data } = await supabase
      .from("invoices")
      .select("id, invoice_number, client_name, status, invoice_total, total, invoice_currency, business_currency, created_at")
  ...
  // Render a capped window of cards; "Show more" expands it. Keeps the DOM
  // light even when a business has hundreds of saved clients.
  const [visibleClientCount, setVisibleClientCount] = useState(24);
  useEffect(() => {
    setVisibleClientCount(24);
  }, [normalizedSearch]);
...
{error && (
  <div className="mx-6 mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">
    {error}
      ...
      {filteredClients.length > visibleClientCount && (
        <div className="text-center">
          <button
            onClick={() => setVisibleClientCount((n) => n + 24)}
          ...
          >
            Show more ({filteredClients.length - visibleClientCount} remaining)
          </button>
        </div>
      )}
