...
type Ticket = {
  ...
  origin?: "manual" | "auto" | "chat" | null; rule_id?: string | null;
...
export default function AdminSupportCenter({ profiles, team }: { profiles: ProfileLite[]; team: Agent[] }) {
  ...
  async function createGithubIssue() {
    if (!selected) return;
    setGithubBusy(true);
    const { data, error } = await supabase.functions.invoke("github-sync", { body: { action: "create_issue", ticketId: selected.id } });
    setGithubBusy(false);
    const result = data as { error?: string; issue_url?: string } | null;
    if (error || result?.error) { setNotice(result?.error || error?.message || "Could not create GitHub issue."); return; }
    setNotice("GitHub issue created and linked.");
    await load();
  }
  ...
  return <section className="space-y-6">
    ...
    <div className="grid min-h-[680px] gap-5 xl:grid-cols-[340px_minmax(0,1fr)_300px]">
      <div className="card overflow-hidden"><div className="space-y-3 border-b p-4"><input className="input" placeholder="Search ticket, user, plan..." value={search} onChange={(e) => setSearch(e.target.value)}/><select className="input" value={view} onChange={(e) => setView(e.target.value as typeof view)}><option value="all">All tickets</option><option value="unassigned">Unassigned</option><option value="urgent">Urgent</option><option value="breached">SLA breached</option></select></div><div className="max-h-[720px] divide-y overflow-y-auto">{filtered.length === 0 ? <p className="p-8 text-center text-sm text-slate-500">No matching tickets.</p> : filtered.map((ticket) => { const timer = sla(ticket); return <button key={ticket.id} onClick={() => setSelectedId(ticket.id)} className={`w-full border-l-4 p-4 text-left hover:bg-slate-50 ${selectedId === ticket.id ? "border-indigo-600 bg-indigo-50/60" : "border-transparent"}`}><div className="flex justify-between gap-2"><b className="line-clamp-2 text-sm">{ticket.subject}</b><span className="flex items-center gap-1">{ticket.origin === "auto" && <span className="rounded-full bg-indigo-100 px-1.5 py-0.5 text-[9px] font-black text-indigo-600" title="Auto-assigned by the automation engine">⚙️ Auto</span>}<span className="text-[10px] font-black uppercase text-slate-400">{ticket.priority}</span></span></div><p className="mt-1 text-xs text-slate-500">{ticket.ticket_number || ticket.id.slice(0,8)} · {ticket.plan_at_creation || "free"}</p><div className="mt-2 flex items-center justify-between gap-2"><span className={`inline-flex rounded-full border px-2 py-1 text-[10px] font-bold ${timer.tone}`}>{timer.label}</span>{ticket.assigned_to && <span className="flex h-5 w-5 items-center justify-center rounded-full bg-indigo-600 text-[10px] font-black text-white" title={agents.find(a => a.id === ticket.assigned_to)?.name || ""}>{(agents.find(a => a.id === ticket.assigned_to)?.name || agents.find(a => a.id === ticket.assigned_to)?.email || "?")[0]?.toUpperCase()}</span>}</div></button>; })}</div></div>
      <div className="card flex min-w-0 flex-col overflow-hidden">{!selected ? <div className="flex flex-1 items-center justify-center p-10 text-slate-500">Select a ticket</div> : <><div className="border-b p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row"><div><p className="text-xs font-black uppercase tracking-wide text-indigo-600">{selected.ticket_number || selected.id}</p><h2 className="mt-1 flex items-center gap-2 text-xl font-black">{selected.subject}{selected.origin === "auto" && <span className="rounded-full bg-indigo-100 px-2 py-0.5 text-[10px] font-black text-indigo-600" title="Auto-assigned by the automation engine">⚙️ Auto</span>}</h2><p className="mt-1 text-sm text-slate-500">{selected.category || "general"} · {date(selected.created_at)}</p></div><span className={`h-fit rounded-full border px-3 py-1 text-xs font-bold ${sla(selected).tone}`}>{sla(selected).label}</span></div></div><div className="max-h-[430px] flex-1 space-y-3 overflow-y-auto bg-slate-50 p-5">{messages.length === 0 && <p className="text-center text-sm text-slate-500">No conversation yet.</p>}{messages.map((message) => <div key={message.id} className={`max-w-[88%] rounded-2xl border p-3 text-sm ${message.is_internal ? "bg-amber-50 border-amber-200" : message.author_type === "customer" ? "ml-auto bg-white border-slate-200" : "bg-indigo-50 border-indigo-200"}`}><div className="mb-1 flex justify-between gap-4 text-[10px] font-bold uppercase text-slate-500"><span>{message.is_internal ? "Internal note" : message.author_type}</span><span>{date(message.created_at)}</span></div><p className="whitespace-pre-wrap">{message.message}</p></div>)}</div>{attachments.length > 0 && <div className="border-t p-4"><p className="mb-2 text-xs font-bold text-slate-500">EVIDENCE</p><div className="flex gap-2 overflow-x-auto">{attachments.map(item => <a key={item.id} href={item.signed_url} target="_blank" rel="noreferrer"><img className="h-20 w-28 rounded-xl border object-cover" src={item.signed_url} alt={item.file_name}/></a>)}</div></div>}<form onSubmit={sendReply} className="space-y-3 border-t p-4"><textarea className="input min-h-24" placeholder={internal ? "Write an internal note..." : "Reply to customer..."} value={reply} onChange={(e) => setReply(e.target.value)}/><div className="flex flex-wrap items-center justify-between gap-3"><label className="text-sm text-slate-600"><input className="mr-2" type="checkbox" checked={internal} onChange={(e) => setInternal(e.target.checked)}/>Internal note</label><button disabled={busy || !reply.trim()} className="btn-primary disabled:opacity-50">{busy ? "Saving..." : internal ? "Add note" : "Send reply"}</button></div></form></>}</div>
      <aside className="space-y-4"><div className="card p-5"><p className="text-xs font-black uppercase text-slate-400">Customer context</p><h3 className="mt-2 font-black">{customer?.business_name || customer?.email || "Unknown customer"}</h3><p className="text-sm text-slate-500">{customer?.email || selected?.user_id || "No linked user"}</p><div className="mt-4 grid grid-cols-2 gap-2 text-xs"><div className="rounded-xl bg-slate-50 p-3"><b>Plan</b><p className="mt-1 capitalize">{customer?.plan || selected?.plan_at_creation || "free"}</p></div><div className="rounded-xl bg-slate-50 p-3"><b>Billing</b><p className="mt-1 capitalize">{customer?.subscription_status || "unknown"}</p></div></div></div>{selected && <div className="card space-y-2 p-5"><p className="text-xs font-black uppercase text-slate-400">GitHub</p>{selected.github_issue_url ? <a href={selected.github_issue_url} target="_blank" rel="noreferrer" className="block w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-center text-sm font-bold text-indigo-700 hover:bg-slate-100">🔗 Issue #{selected.github_issue_number} on GitHub</a> : <button type="button" disabled={githubBusy} onClick={() => void createGithubIssue()} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-50">{githubBusy ? "Creating…" : "🐙 Create GitHub Issue"}</button>}<p className="text-xs text-slate-400">Closing the linked issue on GitHub auto-resolves this ticket.</p></div>}{selected && <div className="card space-y-3 p-5"><p className="text-xs font-black uppercase text-slate-400">Ticket controls</p><label className="block text-xs font-bold text-slate-500">Status<select className="input mt-1" value={selected.status} onChange={(e) => void patchTicket({ status: e.target.value as TicketStatus }, "Status updated.")}>{statuses.map(status => <option key={status} value={status}>{statusLabel[status]}</option>)}</select></label><label className="block text-xs font-bold text-slate-500">Assignee<select className="input mt-1" value={selected.assigned_to || ""} onChange={(e) => void patchTicket({ assigned_to: e.target.value || null }, "Ticket assigned.")}><option value="">Unassigned</option>{agents.map(agent => <option key={agent.id} value={agent.id}>{agent.name || agent.email}</option>)}</select></label>{suggestion && !selected.assigned_to && <button type="button" onClick={() => void patchTicket({ assigned_to: suggestion.id }, "Ticket assigned.")} className="w-full flex items-center justify-between rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700 hover:bg-indigo-100"><span>💡 Suggested: {suggestion.name} ({suggestion.open_count} open)</span><span className="underline">Use</span></button>}<label className="block text-xs font-bold text-slate-500">Priority<div className="mt-1 flex items-center gap-1">{([["low", "⚪", "Low", "bg-slate-100 text-slate-600 border-slate-200"], ["medium", "🟡", "Med", "bg-amber-50 text-amber-700 border-amber-200"], ["high", "🟠", "High", "bg-orange-50 text-orange-700 border-orange-200"], ["urgent", "🔴", "Urgent", "bg-red-50 text-red-700 border-red-200"]] as const).map(([value, dot, label, cls]) => <button key={value} type="button" onClick={() => void patchTicket({ priority: value as Priority }, "Priority updated.")} className={`flex-1 rounded-lg border px-1.5 py-2 text-[11px] font-bold ${selected.priority === value ? cls + " ring-2 ring-offset-1 ring-primary-400" : "bg-white text-slate-400 border-slate-200"}`}>{dot} {label}</button>)}</div></label><textarea className="input min-h-20" placeholder="Required resolution summary" value={resolution} onChange={(e) => setResolution(e.target.value)}/><button disabled={busy} onClick={() => void resolve()} className="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white">Resolve ticket</button></div>}</aside>
    </div>
  </section>;
}
