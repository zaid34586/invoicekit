...
export default function AdminSupportCenter({ profiles, team }: { profiles: ProfileLite[]; team: Agent[] }) {
  ...
  return <section className="space-y-6">
    ...
    <div className="grid min-h-[680px] gap-5 xl:grid-cols-[340px_minmax(0,1fr)_300px]">
      ...
      <aside className="space-y-4"><div className="card p-5"><p className="text-xs font-black uppercase text-slate-400">Customer context</p><h3 className="mt-2 font-black">{customer?.business_name || customer?.email || "Unknown customer"}</h3><p className="text-sm text-slate-500">{customer?.email || selected?.user_id || "No linked user"}</p><div className="mt-4 grid grid-cols-2 gap-2 text-xs"><div className="rounded-xl bg-slate-50 p-3"><b>Plan</b><p className="mt-1 capitalize">{customer?.plan || selected?.plan_at_creation || "free"}</p></div><div className="rounded-xl bg-slate-50 p-3"><b>Billing</b><p className="mt-1 capitalize">{customer?.subscription_status || "unknown"}</p></div></div></div>{selected && <div className="card space-y-3 p-5"><p className="text-xs font-black uppercase text-slate-400">Ticket controls</p><label className="block text-xs font-bold text-slate-500">Status<select className="input mt-1" value={selected.status} onChange={(e) => void patchTicket({ status: e.target.value as TicketStatus }, "Status updated.")}>{statuses.map(status => <option key={status} value={status}>{statusLabel[status]}</option>)}</select></label><label className="block text-xs font-bold text-slate-500">Assignee<select className="input mt-1" value={selected.assigned_to || ""} onChange={(e) => void patchTicket({ assigned_to: e.target.value || null }, "Ticket assigned.")}><option value="">Unassigned</option>{agents.map(agent => <option key={agent.id} value={agent.id}>{agent.name || agent.email}</option>)}</select></label><label className="block text-xs font-bold text-slate-500">Priority<select className="input mt-1" value={selected.priority} onChange={(e) => void patchTicket({ priority: e.target.value as Priority }, "Priority updated.")}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="urgent">Urgent</option></select></label><textarea className="input min-h-20" placeholder="Required resolution summary" value={resolution} onChange={(e) => setResolution(e.target.value)}/><button disabled={busy} onClick={() => void resolve()} className="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white">Resolve ticket</button></div>}</aside>
    </div>
  </section>;
}
