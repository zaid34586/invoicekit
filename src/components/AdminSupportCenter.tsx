...
export default function AdminSupportCenter({ profiles, team }: { profiles: ProfileLite[]; team: Agent[] }) {
  ...
  useEffect(() => { void supabase.from("canned_responses").select("id,title,body").order("created_at").then(({ data }) => setCannedResponses((data as CannedResponse[]) ?? [])); }, []);
