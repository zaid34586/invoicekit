...
type Offer = {
  ...
  paddle_discount_id: string | null; paddle_synced: boolean; created_at: string;
};
...
type FormState = {
  ...
  usage_limit: string; paddle_discount_id: string;
};
...
const emptyForm: FormState = {
  ...
  featured: false, priority: "0", usage_limit: "", paddle_discount_id: "",
};
...
export default function AdminGrowthCenter() {
  ...
  async function save() {
    ...
    const result = editingId
      ? await supabase.from("admin_promo_codes").update(payload).eq("id", editingId)
      : await supabase.from("admin_promo_codes").insert(payload);
    if (result.error) { setNotice(result.error.message); return; }
    setNotice(editingId ? "Offer updated." : "Offer created."); reset(); await loadOffers();
  }
      ...
      <section className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3"><input type="number" value={form.usage_limit} onChange={(e) => setForm({ ...form, usage_limit: e.target.value })} placeholder="Usage limit" className="rounded-2xl border px-3 py-3 text-sm"/><input value={form.paddle_discount_id} onChange={(e) => setForm({ ...form, paddle_discount_id: e.target.value })} placeholder="Paddle discount ID" className="rounded-2xl border px-3 py-3 text-sm"/></div>
