...
import { INDIA_PLANS, GLOBAL_PLANS, type Plan } from "../lib/pricing";
...
export default function UpgradeModal({ isOpen, onClose }: UpgradeModalProps) {
  ...
  const [selected, setSelected] = useState<Extract<Plan, "pro" | "business">>("pro");
  ...
  // Real checkout (region-aware Paddle price IDs, promo codes, etc.) already
  // lives on /billing. Rather than duplicate that logic here, this modal
  // hands off to it with the chosen plan/cycle pre-selected.
  function goToCheckout() {
    onClose();
    navigate(`/billing?plan=${selected}&cycle=${billingCycle}`);
  }
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-[28px] border border-white/70 bg-white shadow-[0_35px_90px_rgba(15,23,42,0.32)] animate-scale-in">
        ...
        <div className="relative px-7 pb-8 pt-8 sm:px-9">
          ...
          <div className="mt-5 text-center">
            ...
            <h2 className="mt-2 text-3xl font-black tracking-[-0.04em] text-slate-950">Rivox {plan.name}</h2>
            <p className="mt-2 text-sm leading-6 text-slate-600">{plan.description}</p>
          </div>

          <div className="mx-auto mt-5 flex w-fit rounded-xl bg-slate-100 p-1">
            {(["pro", "business"] as const).map((p) => (
              <button key={p} onClick={() => setSelected(p)} className={`rounded-lg px-4 py-2 text-sm font-bold capitalize transition ${selected === p ? "bg-white text-slate-950 shadow-sm" : "text-slate-500"}`}>
                {plans[p].name}
              </button>
            ))}
          </div>

          <div className="mx-auto mt-3 flex w-fit rounded-xl bg-slate-100 p-1">
          ...
          <div className="mt-6 text-center">
            <span className="align-top text-xl font-black text-slate-950">{plan.symbol}</span>
          ...
          <button onClick={goToCheckout} className="mt-7 w-full rounded-2xl bg-slate-950 px-5 py-3.5 text-base font-black text-white shadow-xl shadow-slate-950/20 transition hover:-translate-y-0.5 hover:bg-primary-600">
