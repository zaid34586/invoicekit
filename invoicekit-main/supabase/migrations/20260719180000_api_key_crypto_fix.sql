-- Supabase installs pgcrypto in the extensions schema. Explicitly qualify
-- cryptographic helpers so API key generation works with a restricted
...
begin
  ...
  v_raw:='rvx_live_'||encode(extensions.gen_random_bytes(32),'hex');
  insert into public.workspace_api_keys(workspace_id,name,key_prefix,key_hash,created_by)
  values(v_workspace,v_name,left(v_raw,17),encode(extensions.digest(v_raw,'sha256'),'hex'),auth.uid())
