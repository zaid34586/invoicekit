...
Deno.serve(async req=>{if(req.method==="OPTIONS")return new Response("ok",{headers:cors});try{
 ...
 const relation=(key as any)?.workspaces; const {data:ownerProfile}=relation?.owner_user_id?await admin.from("profiles").select("plan").eq("user_id",relation.owner_user_id).maybeSingle():{data:null}; if(!key||key.revoked_at||ownerProfile?.plan!=="business")return respond({error:"Invalid or revoked API key"},401);
